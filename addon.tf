data "aws_eks_addon_version" "this" {
  for_each = { for k, v in merge(var.default_aws_eks_addons, var.additional_aws_eks_addons) : k => v }
...
resource "aws_eks_addon" "eks_addons_to_install" {
  for_each = merge(var.default_aws_eks_addons, var.additional_aws_eks_addons)
